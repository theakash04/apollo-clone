...
export default function DoctorsPage() {
  const { filteredDoctors, isLoading } = useDoctorContext();
  ...
  return (
    <div className="flex flex-col min-h-screen relative ">
      ...
      <div
        className={`absolute inset-0 z-60 justify-center items-center backdrop-blur-2xl overflow-hidden ${
          addIsOpen ? "flex" : "hidden"
        }`}
      >
        <div className="max-w-6xl w-full h-screen">
          <AddDoctor onClose={() => setAddOpen(false)} />
        </div>
      </div>
      <Header />
